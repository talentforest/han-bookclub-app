...
데이터가 필요한 컴포넌트에서 firebase의 실시간 수신 리스너 API를 통해 데이터를 불러오는 구조입니다. 모임 후기를 보여주는 컴포넌트에서 useEffect를 통해 데이터를 불러 recoil의 상태에 저장합니다. 하지만 컴포넌트가 재랜더링되었을 때 이미 필요한 데이터가 상태에 있다면 firebase에서 데이터를 불러오지 않는 구조로 불필요한 데이터 요청을 막았습니다.
