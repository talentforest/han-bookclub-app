...
export default function YearClosingDetail() {
  ...
  const [currTab, setCurrTab] = useState<EventContentsType>('개근상');

  const { clubBookListByYear } = useGetClubByYear(year);

  const { getUserRankList, challengeStatus } = useHandleChallenge(year);

  const findContentResult: (name: EventContentsType) => EventResult =
    useCallback(
      (name: EventContentsType) => {
        const initialResult: EventResult = {
          books: [],
          subjects: [],
          readingLifeQuestions: [],
        };

        if (!club?.meeting?.eventMonth) return initialResult;
  ...
  const initialContentObj: {
    ...
    개근상: {
      name: '독서모임 개근상',
      children: <AbsenceRankList year={year} />,
    },
    챌린지: {
      name: '재독 챌린지',
      children: (
    ...
    모임책: {
      name: '최고의 모임책',
      children: (
        <BestBookList year={year} books={findContentResult('모임책').books} />
      ),
    },
    발제문: {
      name: '최고의 발제문',
      children: (
        <BestSubjectList
          year={year}
          subjects={findContentResult('발제문').subjects}
        />
      ),
    },
    '돌아보는 질문': {
      name: `독서생활을 돌아보는 질문 ${findContentResult('돌아보는 질문').readingLifeQuestions.length}가지`,
      children: (
        <ReadingLifeQuestionList
          year={year}
          questionList={findContentResult('돌아보는 질문').readingLifeQuestions}
        />
      ),
    },
  };
  ...
  return (
    <>
      {(status === 'isLoading' || challengeStatus === 'isLoading') && (
        <LoopLoading className="bg-black" />
      )}
          ...
          <main className="overflow-hidden bg-black pb-40 pt-4">
            <>
              ...
              <ul className="mb-5 mt-24 flex flex-wrap gap-1.5">
                {Object.entries(initialContentObj)
                  .filter(content => content[0] !== '돌아보는 질문')
                  .map(([key, value]) => (
                    <li key={key}>
                      <SquareBtn
                        name={value.name}
                        handleClick={() => setCurrTab(key as EventContentsType)}
                        className={`rounded-t-xl !px-3 !text-sm tracking-tighter ${currTab.includes(key) ? '!bg-[#4d4d4d] !font-bold !text-white' : '!bg-gray4 !text-gray1'}`}
                      />
                    </li>
                  ))}
              </ul>
              ...
              <Section className="min-h-72">
                {initialContentObj[currTab]?.children}
              </Section>

              <Section
                title={initialContentObj['돌아보는 질문'].name}
                className={'!mt-4 [&>h3]:text-white'}
              >
                {findContentResult('돌아보는 질문') &&
                  initialContentObj['돌아보는 질문'].children}
              </Section>
            </>
          </main>
        </>
